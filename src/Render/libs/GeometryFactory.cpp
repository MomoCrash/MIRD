...
void GeometryFactory::Initialize(ID3D12Device* pDevice, ID3D12GraphicsCommandList* pCommandList)
{
	mpDevice = pDevice;
	mpCommandList = pCommandList;
	
	Cube	=	CreateBox(1.0f, 1.0f, 1.0f, 3);
	Geosphere = CreateGeosphere(1.0f, 5);
	Grid	=	CreateGrid(1.0f, 1.0f, 10, 10);
	Sphere	=	CreateSphere(1.0f, 10, 10);
}

GeometryFactory::GeometryFactory(): mpDevice(nullptr), mpCommandList(nullptr), Cube(nullptr), Geosphere(nullptr),
                                    Grid(nullptr), Sphere(nullptr) { }
...
Mesh* GeometryFactory::GetCube()
{
	return instance().Cube;
}

Mesh* GeometryFactory::GetGeosphere()
{
	return instance().Geosphere;
}

Mesh* GeometryFactory::GetGrid()
{
	return instance().Grid;
}

Mesh* GeometryFactory::GetSphere()
{
	return instance().Sphere;
}

Mesh* GeometryFactory::CreateBox(float width, float height, float depth, uint32 numSubdivisions)
{
    MeshData meshData;
...
Mesh* GeometryFactory::CreateSphere(float radius, uint32 sliceCount, uint32 stackCount)
{
    MeshData meshData;
...
Mesh* GeometryFactory::CreateGeosphere(float radius, uint32 numSubdivisions)
{
    MeshData meshData;
...
Mesh* GeometryFactory::CreateGrid(float width, float depth, uint32 m, uint32 n)
{
    MeshData meshData;
...
Mesh* GeometryFactory::CreateQuad(float x, float y, float w, float h, float depth)
{
    MeshData meshData;
...
Mesh* GeometryFactory::LoadGeometryFromFile(std::wstring path, bool invertV)
{

	MeshData data;
	
	std::fstream meshFile;
	meshFile.open(CoreUtils::GetResourceFolder() + L"objects/" + path, std::ios::in);

	bool isFirst = true; 
	
	std::vector<XMFLOAT2> tempTexCoords;
	std::vector<XMFLOAT3> tempPosition;
	...
	for(std::string line; std::getline(meshFile, line); )   // Read stream line by line
	{
	    ...
	    { 
	        float u, v;
	        in >> u >> v;
	        tempTexCoords.push_back({u, invertV ? 1 -v : v}); // Flip V to match DirectX/OpenGL convention
	    } 
	    else if (type == "f") 
	    {
	        ...
	        while (in >> indicesString) 
	        {
	            ...
	            if (!vertexCache.contains(key)) 
	            {
	            	...
	            	if (vt != -1 && vt < tempTexCoords.size()) newVertex.TexC = tempTexCoords[vt];
	...
	instance().GenerateGeometryBuffer(geometry);
...
void GeometryFactory::GenerateGeometryBuffer(Mesh* geo)
{
	...
	geo->VertexBufferGPU->SetName(L"VERTEX_BUFFER");
	...
	geo->IndexBufferGPU->SetName(L"INDEX_BUFFER");
